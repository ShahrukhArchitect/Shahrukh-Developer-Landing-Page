        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              // bg-white nikal kar direct dark background lock kar diya
              className="relative p-8 rounded-3xl bg-[#0B0E14] border border-white/5 group hover:border-[#FFD700]/30 transition-all"
            >
              {/* Step Number - Opacity adjusted for subtle look on dark bg */}
              <span className="text-5xl font-black text-white/5 absolute top-4 right-6 group-hover:text-[#FFD700]/10 transition-colors">
                {step.num}
              </span>

              {/* Heading - text-white force kar diya */}
