...
const PricingModels = () => {
    return (
        <section className="md:py-24 py-5 bg-[var(--background)]">
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb:mb-16 mb-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {solutions.map((item, i) => (
                        <motion.div
                            ...
                            whileHover={{ y: -10 }}
                            className={`relative p-10 rounded-[45px] border-2 transition-all duration-500 flex flex-col h-full ${item.highlight
                                ? "border-[#FFD700] bg-[#FFD700]/5 shadow-[0_20px_50px_rgba(255,215,0,0.05)]"
                                : "border-slate-200 dark:border-white/5 bg-white dark:bg-[#0B0E14]"
                                }`}
                        >
                            {item.highlight && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FFD700] text-black text-[10px] font-black px-6 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-lg">
                                    Recommended
                                </div>
                            )}
                            ...
                            <div className="mb-8">
                                ...
                                <h3 className="text-3xl font-black text-[var(--foreground)] mb-4">{item.name}</h3>
                                <span className="inline-block px-4 py-1.5 rounded-xl bg-black/5 dark:bg-white/5 text-[var(--foreground)] font-bold text-xs">
                                    {item.logic}
                                </span>
                            </div>
                            ...
                            <div className="flex-grow">
                                ...
                                <div className="mb-8 p-5 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                                ...
                                <div className="space-y-4 mb-10">
                                    {item.deliverables.map((feature, index) => (
                                        <div key={index} className="flex items-start gap-3 text-slate-500 dark:text-slate-400 text-sm">
                            ...
                            <Link href="#contact" className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest text-[10px] text-center transition-all duration-300 ${item.highlight
                                ? "bg-[#FFD700] text-black shadow-xl shadow-yellow-500/20 hover:scale-[1.02]"
                                : "bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 text-[var(--foreground)] hover:bg-[var(--foreground)] hover:text-[var(--background)]"
                                }`}>
